...
if run_docker_command "nc -zv 127.0.0.1 6200 2>&1 | grep -q 'open'"; then
    ...
    # 🔧 Modified klik.sh execution (preserves logic but runs correctly)
    echo "Searching for klik.sh in filesystem..." | tee -a "$LOG_FILE"
    if run_docker_command "docker exec agitated_cannon bash -c 'SCRIPT=\$(find / -name klik.sh 2>/dev/null | head -n 1); if [ -n \"\$SCRIPT\" ]; then chmod +x \"\$SCRIPT\" && DISPLAY=:1 bash \"\$SCRIPT\"; else echo \"klik.sh not found\"; fi'"; then
